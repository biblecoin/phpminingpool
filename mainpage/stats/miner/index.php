...
<body class="blog-home-page">   
    <div class="header-wrapper header-wrapper-blog-home">
        ...
        <header id="header" class="header navbar-fixed-top">  
            <div class="container">       
                <h1 class="logo">
                    <a href="../"><span class="highlight">Biblecoin</span>Pool.co</a>
                </h1><!--//logo-->
                <nav class="main-nav navbar-right" role="navigation">
                    <div class="navbar-header">
                        <button class="navbar-toggle" type="button" data-toggle="collapse" data-target="#navbar-collapse">
...
echo '<tr><td>Minute</td><td>'.sprintf('%f', $rev_min).' bible</td><td>'.sprintf('%f', $rev_min*$eth_price_usd).' $</td></tr>';
echo '<tr><td>Hour</td><td>'.sprintf('%f', $rev_hour).' bible</td><td>'.sprintf('%f', $rev_hour*$eth_price_usd).' $</td></tr>';
echo '<tr><td>Day</td><td>'.sprintf('%f', $rev_day).' bible</td><td>'.sprintf('%f', $rev_day*$eth_price_usd).' $</td></tr>';
echo '<tr><td>Week</td><td>'.sprintf('%f', $rev_week).' bible</td><td>'.sprintf('%f', $rev_week*$eth_price_usd).' $</td></tr>';
echo '<tr><td>Month</td><td>'.sprintf('%f', $rev_month).' bible</td><td>'.sprintf('%f', $rev_month*$eth_price_usd).' $</td></tr>';
echo '</table>';
...
while ($row=mysqli_fetch_row($existResult)){
    ...
    $r++;
    if ($r == $count_r) {
        echo '<b><br>Last 50 Payments</b><tr><td>'.$balanceforPayment->toString().' wei<br>'.$etherforPayment.' ether<br>When: '.gmdate("Y-M-d  h:i:s", $row[1]).'<br>Biblecoin network fee deduced:'.$row[3].'<br>TXID:<a href="http://etherscan.io/tx/'.$row[2].'" target="_blank">'.$row[2].'</a>';
    } else {
        echo '<tr><td>'.$balanceforPayment->toString().' wei<br>'.$etherforPayment.' ether<br>When: '.gmdate("Y-M-d  h:i:s", $row[1]).'<br>Biblecoin network fee deduced:'.$row[3].'<br>TXID:<a href="http://etherscan.io/tx/'.$row[2].'" target="_blank">'.$row[2].'</a>';
    }
    echo '</td></tr>';
}
