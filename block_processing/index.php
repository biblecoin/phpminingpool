...
while(1) {
	...
	/////HANDLE CARSH
	if (!$coinbase) {
		echo "\nCan't get coinbase - restarting bible";
		$current .= "\nCant get coinbase - restarting bible";
		//$restart_eth = exec('screen bible -b -j --json-rpc-port 8983 > /dev/null &');
	} else {
	///////////////
  ...
  curl_setopt($ch1, CURLOPT_HTTPHEADER, array(                                                                          
	...
	while ($row=mysqli_fetch_row($existResultMinersss)){
			...
			if ($minedBy == $coinbase) {
				...
				$output=0;
				$output = shell_exec('/usr/bin/python /root/pybiblecoin/biblecoin/nonce_fast.py '.$filtempe.' '.rand(5,123142312));
				$test = explode(' ', $output);
